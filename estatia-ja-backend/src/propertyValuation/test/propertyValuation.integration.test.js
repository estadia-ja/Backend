...
describe('PropertyValuation Routes - Integration test', () => {
    ...
    describe('DELETE /property-valuation/valuationId', () => {
        it('Should delete a valuation', async () => {
            ...
            const response = await request(app)
                // CORRIGIDO (Erro 2): Rota aninhada completa
                .delete(`/reserve/${reserve1.id}/property-valuation/${valuation.id}`)
                .set('Authorization', `Bearer ${renterToken}`) // 'renter' é o dono da 'reserve1'
        ...
        it('should return 403 if a different user tries to delete a valuate', async () => {
            const valuation = await prisma.propertyValuation.create({
                data: {
                    reserveId: reserve1.id, // Esta avaliação pertence a 'reserve1' (do 'renter')
                    noteProperty: 5,
                    commentProperty: 'Ótima estadia!',
                }
            });
            ...
            const response = await request(app)
                // CORRIGIDO (Erro 2): Rota aninhada completa
                .delete(`/reserve/${reserve1.id}/property-valuation/${valuation.id}`)
                .set('Authorization', `Bearer ${randomToken}`) // Token de usuário aleatório
    
            // CORRIGIDO (Erro 3): Esperar 403
            expect(response.status).toBe(403); 
            // Agora sim podemos checar o body
            expect(response.body.error).toContain('Ação não autorizada'); // Ajuste a mensagem para ser a exata do seu controller
        });
    });
});