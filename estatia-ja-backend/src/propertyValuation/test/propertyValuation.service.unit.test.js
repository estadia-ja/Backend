...
describe('test propertyValuation service', () => {
    ...
    describe('createPropertyValuation', () => {
        ...
        it('should create a new valuation succefully', async () => {
            ...
            const result = await propertyValuationService.createPropertyValuation(reserveId, userId, valuationData);
        ...
        it('should thorw error if user is not authorized', async () => {
            ...
            await expect(propertyValuationService.createPropertyValuation(reserveId,userId,valuationData))
                .rejects.
                toThrow('Ação não autorizada. Você não pode avaliar uma reserva que não é sua.');
        ...
        it('should throw error if reserve is already valuated', async () => {
            ...
            await expect(propertyValuationService.createPropertyValuation(reserveId,userId,valuationData))
                .rejects
                .toThrow('Esta reserva já foi avalia.');
    ...
    describe('getValuationByProperty', () => {
        ...
        const mockValuations = [
            { id: 'val-1', noteProperty: 5, reserve: { user: { id: 'u1', name: 'Alice' } } },
            { id: 'val-2', noteProperty: 4, reserve: { user: { id: 'u2', name: 'Bob' } } },
        ];
        ...
        it('should thorw error if no baluations are found', async () => {
            ...
            await expect(propertyValuationService.getValuationsByProperty(propertyId))
                .rejects
                .toThrow("Nenhuma avaliação encontrada para este imóvel");
    ...
    describe('deletePropertyValuation', () => {
        ...
        it('should delete a valuation succefully', async () => {
            ...
            expect(prisma.propertyValuation.findUnique).toHaveBeenCalledWith({where: { id: valuationId },include: { reserve: true },});
            expect(prisma.propertyValuation.delete).toHaveBeenCalledWith({where: { id: valuationId },});
        });

        it('should throw error if valuation does not exist', async () => {
            prisma.propertyValuation.findUnique.mockResolvedValue(null);
        
            await expect(propertyValuationService.deletePropertyValuation(valuationId, userId))
                .rejects
        ...
        it('should throw error if user is not authorized to delete', async () => {
            ...
            await expect(propertyValuationService.deletePropertyValuation(valuationId, userId))
            .rejects
            .toThrow('Ação não autorizada. Vacê não pode deletar uma avaliação que não é sua.');
