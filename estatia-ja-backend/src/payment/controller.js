...
const paymentController = {
    async create(req, res){
        try {
            ...
            const payment = await paymentService.createPayment(reserveId, userId, paymentData)
            res.status(200).json(payment.toJSON());
        } catch (error) {
            if (error.message.includes("Esta reserva já foi paga.")) {
                return res.status(409).json({ error: error.message });
            }
            if (error.message.includes("Ação não autorizada. Esta reserva não pertence a você.")) {
