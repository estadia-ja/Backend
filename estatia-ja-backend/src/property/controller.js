...
const propertyController = {
  ...
  async findRanked(req, res) {
    try {
      const properties =
        await propertyService.findPropertiesRankedByValuation();
      res.status(200).json(properties.map((property) => property.toJSON()));
    } catch (error) {
  ...
  async updateData(req, res) {
    try {
      ...
      const updatedProperty = await propertyService.updatePropertyData(
        id,
        validatedData,
        userId
      );
  ...
  async updateImages(req, res) {
    try {
      ...
      const imageBuffers = files ? files.map((file) => file.buffer) : [];
      const userId = req.user.id;
      const updateProperty = await propertyService.updatePropertyImages(
        id,
        imageBuffers,
        userId
      );
