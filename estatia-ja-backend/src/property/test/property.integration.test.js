...
describe('Property Routes - Integration Tests', () => {
  ...
  describe('POST /property', () => {
    it('should create a new property with images', async () => {
      const response = await request(app)
        ...
        .attach('images', Buffer.from('fake-image-data-1'), 'test-image1.jpg');
  ...
  describe('PUT /property/:id', () => {
    it('should update property data if user is the owner', async () => {
      const property = await prisma.property.create({
        data: { ...propertyData, userId: testUser.id },
      });
      const updateData = {
        ...propertyData,
  ...
  describe('PUT /property/:id/images', () => {
    it('should update (replace) images if user is the owner', async () => {
      ...
      await prisma.propertyImage.create({
        data: { image: Buffer.from('old-image'), propertyId: property.id },
      });
      const response = await request(app)
    ...
    it('should remove all images if an empty array is sent', async () => {
      ...
      const imagesInDb = await prisma.propertyImage.findMany({
  ...
  describe('DELETE /property/:id', () => {
    ...
    it('should return 401 if no token is provided', async () => {
      ...
      const response = await request(app).delete(`/property/${property.id}`);
  ...
  describe('GET /property/available', () => {
    ...
    it('should find available properties just between dates (and ignore others)', async () => {
      ...
      const response = await request(app).get(
        `/property/available?dateStart=${dateStart}&dateEnd=${dateEnd}`
      );
    ...
    it('should find available properties filtering by state and guests', async () => {
      ...
      const response = await request(app).get(
        `/property/available?dateStart=${dateStart}&dateEnd=${dateEnd}&state=${state}&guests=${guests}`
      );
    ...
    it('should return 400 if dates are missing', async () => {
      const response = await request(app).get(
        '/property/available?dateStart=2025-11-12T00:00:00Z'
      );
  ...
  describe('GET /property/city/:city', () => {
    ...
    it('should return properties in a specific city with their avgRating', async () => {
      const res1_prop1 = await prisma.reserve.create({
        data: {
          ...
          dateEnd: new Date(),
          status: 'FINALIZADA',
          propertyId: propSP1.id,
          userId: otherUser.id,
        },
      });
      await prisma.propertyValuation.create({
        data: { noteProperty: 5.0, reserveId: res1_prop1.id },
      });
      ...
      await prisma.propertyValuation.create({
        data: { noteProperty: 3.0, reserveId: res2_prop1.id },
      });
      ...
      const res1_prop2 = await prisma.reserve.create({
        data: {
          ...
          dateEnd: new Date(),
          status: 'FINALIZADA',
          propertyId: propSP2.id,
          userId: otherUser.id,
        },
      });
      await prisma.propertyValuation.create({
        data: { noteProperty: 5.0, reserveId: res1_prop2.id },
      });
    ...
    it('should return 404 if no properties are found in that city', async () => {
      const response = await request(app).get(
        '/property/city/CidadeInexistente'
      );
  ...
  describe('GET /property/rank-by-valuation', () => {
    it('should return properties ranked by average valuation', async () => {
      ...
      const response = await request(app).get('/property/ranked-by-valuation');
