...
describe('test property service', () => {
  ...
  describe('createProperty', () => {
    it('should return a new property', async () => {
      ...
      const imageBuffers = [Buffer.from('img1'), Buffer.from('img2')];
      ...
      mockTx.property.create.mockResolvedValue({
        id: newPropertyId,
        ...propertyTest,
        userId,
      });
      ...
      vi.spyOn(propertyService, 'getPropertyById').mockResolvedValueOnce(
        propertyWithImages
      );
      ...
      expect(mockTx.property.create).toHaveBeenLastCalledWith({
        data: { ...propertyTest, userId },
      });
      ...
      expect(propertyService.getPropertyById).toHaveBeenCalledWith(
        newPropertyId
      );
  ...
  describe('getAllImagesForProperty', () => {
    ...
    it('should throw "Imóvel sem imagem" if no images are found', async () => {
      ...
      await expect(
        propertyService.getAllImagesForProperty(propertyId)
      ).rejects.toThrow('Imóvel sem imagem');
    });
  });
  ...
  describe('findAvailableProperties', () => {
    it('should build query for dates only', async () => {
      ...
      const result = await propertyService.findAvailableProperties(
        dateStart,
        dateEnd,
        undefined,
        undefined
    ...
    it('should throw an error if dates are missing', async () => {
      await expect(
        propertyService.findAvailableProperties(null, '2025-11-15T10:00:00Z')
      ).rejects.toThrow('as datas de início e fim são obrigatórias.');

      await expect(
        propertyService.findAvailableProperties(
  ...
  describe('getPropertiesByCity', () => {
    ...
    it('should throw an error if no properties are found', async () => {
      ...
      await expect(propertyService.getPropertyByCity(testCity)).rejects.toThrow(
        'Nenhum imóvel encontrado nesta cidade'
      );
  ...
  describe('updatePropertyData', () => {
    it('should update property data successfully', async () => {
      ...
      const finalPropertyInstance = new Property({
        ...mockUpdatedPropertyRaw,
        avgRating: 4.5,
      });
      ...
      vi.spyOn(propertyService, 'getPropertyById').mockResolvedValueOnce(
        finalPropertyInstance
      );

      const result = await propertyService.updatePropertyData(
        propertyId,
        updateData,
        ownerId
      );
    ...
    it('should throw an error if property does not exist.', async () => {
      ...
      await expect(
        propertyService.updatePropertyData(propertyId, updateData, userId)
    ...
    it('should return an error if user is not the owner', async () => {
      ...
      await expect(
        propertyService.updatePropertyData(propertyId, updateData, wrongUserId)
  ...
  describe('updatePropertyImages', () => {
    it('should delete a image and create a new', async () => {
      ...
      expect(prisma.$transaction).toHaveBeenCalledOnce();
      expect(mockTx.propertyImage.deleteMany).toHaveBeenCalledWith({
        where: { propertyId: propertyId },
      });
      expect(mockTx.propertyImage.createMany).toHaveBeenCalledWith({
    ...
    it('should throw an error if property does not exist', async () => {
      ...
      await expect(
        propertyService.updatePropertyImages(
          propertyId,
          newImageBuffers,
          userId
        )
  ...
  describe('deleleProperty', () => {
    it('should delete a property', async () => {
      ...
      await expect(
        propertyService.deleleProperty(propertyId, ownerId)
    ...
    it('should return an error if property does not exist', async () => {
      ...
      await expect(
        propertyService.deleleProperty(propertyId, userId)
      ).rejects.toThrow('Imóvel não existe');
    ...
    it('should return an error if user is not the owner', async () => {
      ...
      await expect(
        propertyService.deleleProperty(propertyId, wrongUserId)
      ).rejects.toThrow('Ação não autorizada');
