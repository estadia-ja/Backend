...
describe('test reserve service', () => {
    ...
    describe('createReserve', () => {
        ...
        const mockNewReserve = { id: 'reserve-1', propertyId, userId, ...reserveData, status: 'CONFIRMADA', property: mockProperty, user: { id: userId, name: 'Guest' } };
        ...
        it('should create a reserve if property is avalaible', async () => {
            ...
            const result = await reserveService.createReserve(propertyId, userId, reserveData);
            ...
            expect(prisma.reserve.create).toHaveBeenCalledWith({
                data: { propertyId, userId, dateStart: reserveData.dateStart, dateEnd: reserveData.dateEnd, status: 'CONFIRMADA' },
        ...
        it('should throw an error if property does not exist', async () => {
            ...
            await expect(reserveService.createReserve(propertyId, userId,reserveData))
                .rejects
                .toThrow('Imóvel não existe');
        });

        it('should throw an error if user tries to reserve their own property', async () => {
            prisma.property.findUnique.mockResolvedValue({ ...mockProperty, userId: userId });
            await expect(reserveService.createReserve(propertyId, userId,reserveData))
                .rejects
                .toThrow('Você não pode reservar seu própio imóvel.');
        });
        ...
        it('should throw an error if there is a date conflict', async () => {
            ...
            await expect(reserveService.createReserve(propertyId, userId,reserveData))
                .rejects
                .toThrow('O imóvel já está reservado para estas datas');
        });
    });
    ...
    describe('updateReserve', ()=> {
        ...
        it('should update a reserve successfully', async () => {
            ...
            prisma.reserve.findFirst.mockResolvedValue(null);
            prisma.reserve.update.mockResolvedValue({ ...mockReserve, ...updateData});

            const result = await reserveService.updateReserve(reserveId, userId, updateData);

            expect(prisma.reserve.findUnique).toHaveBeenCalledWith({ where: { id: reserveId } });
        ...
        it('should throw an error if user is not the owner of the reserve', async () => {
            prisma.reserve.findUnique.mockResolvedValue(mockReserve);
            await expect(reserveService.updateReserve(reserveId, 'wrong-user-id', updateData))
                .rejects
                .toThrow('Ação não autorizada, a reserva não é sua');
        });

        it('should throw an error if the reserve is not exist', async () =>{
            prisma.reserve.findUnique.mockResolvedValue(null);

            await expect(reserveService.updateReserve(reserveId, userId, updateData))
                .rejects
        ...
        it('should throw an error if the user is not the owner of the reserve', async () => {
            ...
            await expect(reserveService.updateReserve(reserveId, wrongUserId, updateData))
                .rejects
                .toThrow('Ação não autorizada, a reserva não é sua');
        });
        ...
        it('should throw an error if the reservation has already started', async () => {
            ...
            await expect(reserveService.updateReserve(reserveId, userId, updateData))
                .rejects.toThrow(/data de início não pode ser no passado|Não é possível alterar uma reserva que já começou/);
    ...
    describe('cancelReserve', () => {
        ...
        it('should allow the guest to cancel their own reserve', async () => {
            ...
            await expect(reserveService.cancelReserve(reserveId, guestId))
                .resolves
                .toBeUndefined();
        ...
        it('should allow the property owner to cancel a reserve', async () => {
            ...
            await expect(reserveService.cancelReserve(reserveId, ownerId))
                .resolves
                .toBeUndefined();
        ...
        it('should throw an error if a third party tries to cancel', async () => {
            prisma.reserve.findUnique.mockResolvedValue(mockReserve);
            await expect(reserveService.cancelReserve(reserveId, 'third-party-id'))
